    ...
    if (message is List) {
      ...
      for (var i = 0; i < operations.length; i++) {
        ...
        if (operation.operation == UserActionOperation.CODE) {
          ...
          Iterable<BaseOperation> _repeatSequence = operations.getRange(
      ...
      bool? available = await waitForIdle(
          PumpWaterOperation(
              currentIndex: 0, duration: 0, targetTemperature: 28),
          _device);
...
Future<bool?> waitForIdle(BaseOperation operation, DeviceStats server) async {
  ...
  var json = (operation is AdvancedOperation)
      ? operation.toAdvancedJson()
      : operation.toJson();
  print("sending $json");
