...
class OperationTemplateItemDataAccess implements DataAccess<OperationTemplateItem>{
  ...
  Future<int?> delete(int id) async{
    return database?.delete(OperationTemplateItem.tableName(), where: "id = ?", whereArgs: [id]);
  }

  @override
  Future<List<OperationTemplateItem>?> findAll() async{
    var list = await database?.rawQuery('SELECT * FROM ${OperationTemplateItem.tableName()}');
  ...
  Future<OperationTemplateItem?> getById(int id) async{
    return (await database
        ?.query(OperationTemplateItem.tableName(), where: "id = ?", whereArgs: [id]))
        ?.map((e) => OperationTemplateItem.fromDatabase(e))
        .first;
  }

  @override
  Future<List<OperationTemplateItem>?> search(String? where, {List<Object>? whereArgs}) async{
  ...
  @override
  Future<OperationTemplateItem?> updateById(int id, OperationTemplateItem t) async{
