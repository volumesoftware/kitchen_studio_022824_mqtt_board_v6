...
class OperationTemplateDataAccess implements DataAccess<OperationTemplate>{
  ...
  Future<int?> delete(int id) async{
    return database?.delete(OperationTemplate.tableName(), where: "id = ?", whereArgs: [id]);
  }

  @override
  Future<List<OperationTemplate>?> findAll() async{
    var list = await database?.rawQuery('SELECT * FROM ${OperationTemplate.tableName()}');
  ...
  Future<OperationTemplate?> getById(int id) async{
    return (await database
        ?.query(OperationTemplate.tableName(), where: "id = ?", whereArgs: [id]))
        ?.map((e) => OperationTemplate.fromDatabase(e))
        .first;
  }

  @override
  Future<List<OperationTemplate>?> search(String? where, {List<Object>? whereArgs}) async{
