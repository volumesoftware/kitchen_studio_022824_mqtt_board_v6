...
class DeviceDataAccess implements DataAccess<DeviceStats> {
  ...
  Future<List<DeviceStats>?> findAll() async {
    var list = await database?.rawQuery('SELECT * FROM ${DeviceStats.tableName()}');
  ...
  Future<DeviceStats?> updateById(int id, DeviceStats t) async {
    int? count = await database?.update(
        DeviceStats.tableName(), t.toJson(),
        where: "id = ?",
        whereArgs: [id]);
  ...
  @override
  Future<List<DeviceStats>?> search(String? where, {List<Object>? whereArgs}) async {
